...
def plot_test():
    ...
    x_widget = widgets.FloatSlider(min=0.0, max=100.0, step=1, value=50)
    y_widget = widgets.FloatSlider(min=0.0, max=100.0, step=1, value=60)

    def sync_widget(*args):
        x_widget.value = 100 - y_widget.value
    y_widget.observe(sync_widget, 'value')    

    def update_plot(x, y):
        print(x, y)
        line.set_ydata(y * np.ones_like(np.linspace(0, 100)))
        print(line.get_ydata())
        fig.canvas.draw()

    interact(update_plot, x=x_widget, y=y_widget)
